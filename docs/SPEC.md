...
当前仓库只建设“单股自然语言策略回测”纵切：用户给出一只股票和一句买卖规则，系统将其编译为受限策略，并用版本化数据、A 股执行规则和资金账本完成历史回测与因果解释。
...
AI 在这里是规则编译器，不是收益预测器。用户确认后的策略必须确定、版本化、可重放。
...
用户问题固定为：“如果当前股票按这条规则买卖，过去会怎样？”

输入是单只股票、自然语言交易规则、回测区间和可编辑执行假设；输出是策略收益、同约束买入持有基准、风险、交易列表和完整交易因果轨迹。当前状态为 `implemented / fixture-tested / Mock-verified / Live-unverified`。
...
当前不承诺：股票池、组合、融资融券、实盘下单、自动寻优、用户 Python/SQL、Tick/L2 队列、任意自然语言、收益预测或投资建议。
...
1. AI 负责翻译，不负责自由发挥。
2. 完整表达不追问；只有缺失信息会实质改变结果时，集中澄清一次。
...
首屏不能出现完整参数表、成交成本表、数据来源长说明、运行证据或多张独立悬浮卡。它应有对话感，但不是聊天机器人：用户输入是策略指令，不是与虚拟人格闲聊。
...
当前真实后端在缺少股票、缺买入条件或缺卖出条件时，分别只返回一次方向正确的集中澄清，并让用户编辑原话。系统没有“金叉买、死叉卖”等统一默认交易策略；统一的只有资金、费用、容量、有效期等版本化执行默认。产品目标还允许在“盘中还是收盘确认”等会改变执行语义的问题上集中澄清一次，但前提是两个选项都存在真实可执行链路。
...
| 状态 | 必须告诉用户什么 | 下一步 |
|---|---|---|
| 无法识别策略 | 哪一部分无法映射到已发布规则 | 换一个已支持表达或查看示例 |
...
股票和时间语义门禁。当前没有正式模型 transport、bootstrap 开关、原文 span/模型/提示/校验结果的完整持久化，
也没有真实 API/H5 证据，因此整体仍是 `implemented / fixture-tested / Live-unverified`。模型不能生成或执行
Python、SQL、Pine Script 或任意代码；不能把 adapter 已存在写成已经理解任意表达。
...
- 已部分买入后按“单持仓、不加仓”停止补单，DAY 剩余量过期；不能次日重新下原始全量；
...
| 持续状态（RSI<30、价格高于 MA） | 单次有效，不叫重试；每个完成 session 重新计算，仍为真才生成新 signal/version 和新 DAY 单 |
