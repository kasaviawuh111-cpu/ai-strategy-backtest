...
- `trading_plan`（网格、定投、条件计划）不属于指标买卖规则：审阅卡只说明可理解的计划行为，不展示基准价、格距、股数等执行参数；标题不得拼接底层建仓或成交条款。参数页不渲染空的“交易规则与参数”买/卖栏，而是在“高级设置”内展示“交易计划与参数”。交易计划内的费率、滑点和最低佣金是该计划唯一可编辑的成本口径；不得再显示一套不参与计划回测的通用“成交与费用”或“研究执行参数”。普通条件策略才使用“交易规则与参数”。
