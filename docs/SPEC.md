...
## 2026-09-16 补充：未指定阈值与多轮承接

- 对已识别的指标条件，用户未指定数值阈值时允许补充合理建议，生成可编辑方案；在回复中明确写明“系统建议，可修改”，不再要求先确认阈值才能查看方案。
- 系统补充不等于用户原话或Catalog默认值；保留字段校验、指标能力与用户明确数值、方向、组合关系的校验，不能借补齐参数修改用户条件。补齐不自动启动回测，仍由用户点击开始回测。
- 历史会话仅因旧版“阈值尚未明确”门禁待确认时，用户回复“可以，按你说的来”“是”或唯一建议阈值即可接受已展示方案；保留股票、其他规则及执行设置。其他语义冲突不能由该兼容路径直接放行。
- 用户后续修改参数按当前策略承接；仅在确有多个待改字段且无法唯一定位时澄清，不重复追问已经回答的同一问题。

### 有依据的回测区间调整

默认/相对区间以实际数据末日为锚点；用户明确填写的日期以服务器可信今天为未来日期校验上界，两者不得混用。已过数据末日但不晚于今天的显式日期必须保留到数据预检，由范围协商流程处理，不能提前报模型解析失败或默改日期。

分钟来源先完成私有文件、已固定快照及接口补取。补取失败不得推断范围不足；补取结束后，仅当已读取交易日的缺口位于首尾时，才可形成候选边界，中间断档不截断规避。依赖前一交易日容量的方案须保留真实前置日。候选边界还需经过同一完整预检，不能用文件最早/最晚日期替代完整性验证。

日期不足与接口故障、经济含义校验异常分开。对于结束日晚于实际导入水位、或请求历史早于接口核实上市日的情况，先生成候选区间（起点须保留实际预热长度），再对同一策略重新执行完整数据准备。只有全部依赖校验通过，才显示原日期、已预填建议日期及“接受建议范围”。接受前保存原策略作为修订基础，不启动回测；接受后仅调整日期，仍由用户点击开始回测。建议范围是已验证可用的区间，不宣称是所有来源的最大覆盖；不从报错文字或全市场水位推断某股票完整覆盖。
...
- 组合指同一只沪深A股的买卖规则协作，不是多股票资产组合；所有规则共享一份现金、持仓、成本、费用与T+1约束。
- 用户未声明先后时，不按句子中的买卖排列臆造阶段。独立计划保留各侧执行时点；明确“成交后再执行”才建立顺序依赖。条件“且／或”按原关系保留。
- 参数编辑、保存修订及报告使用同一策略规范。修改一侧不得丢失另一侧；定投附带退出规则在高级设置中必须可见可编辑，并保存至exit_rules。
- 单计划、独立买卖双计划、计划与指标组合复用既有执行器；数据不足不降低频率、不缩短区间或替换规则。触发价格不保证实际成交价格。
- 本地开发和代表案例证据见[组合开发记录](combination-development-20260916.md)。记录中的合成测试、实时模型、真实行情、浏览器验证分别标记，不互相替代，不据此声明全部组合或公网完成。
...
#### 4.5.1 策略审阅与参数页 UI 规范

信息分层固定为“审阅策略 → 编辑具体规则 → 调整高级执行设置”：

- 审阅卡展示股票、买/卖方向、条件关系和足以核对策略含义的完整规则句。阈值、价格、持有天数等决定规则含义的值必须保留，例如“RSI 低于 50”和“持有 10 个交易日退出”；不得精简成语义不完整的“RSI 低于”或“持有期退出”。
- 审阅卡不再追加重复的参数明细行，不展示周期、定义版本、计算口径、滑点、费用和容量等二级技术参数。具体条件行是参数二级页入口；二级页展示当前规则的完整参数与可编辑控件。
- 参数页沿用现有白底、发丝描边、12px 分组圆角和黑色主操作。不使用大面积浅灰块包住全部字段，分组优先靠标题、间距与 1px 分隔线建立。
- 字体层级只允许五级：页面标题使用现有衬线标题字；分组标题 14px/600；条件标题 16px/600；字段标签 14px/500；辅助说明 12px/400。表单值为 16px/400，数字使用等宽数字特性。
- 字段行桌面端使用稳定的“标签列 + 控件列”，窄屏改为上下排列；控件可见高度不低于 44px，信息图标保留可触摸区但不得显示成独立大按钮。
- “交易规则”和“指标参数”合并为“交易规则与参数”：每条买卖规则的标题、触发方式和参数在同一卡片内核对、编辑，不得先重复展示一遍规则，再在另一分组查找对应参数。重复的通用参数名必须补齐指标上下文，例如“KDJ 周期”“RSI 周期”。
- 当前条件和回测区间默认展开；成交、费用、容量、预热、结算延长等低频研究设置归入“高级设置”并默认折叠。审阅卡入口统一命名为“高级设置”，不得另设“成交设置”入口。
- 会改变成交次数的执行语义不得因折叠而消失。高级设置必须明确显示“触发口径”和“后续买入”：上穿/下穿/金叉/死叉只在新事件发生时触发，条件持续成立不重复触发；采用 `accumulate_on_new_entry_signal` 时，持仓期间出现新的同类触发可以再次买入。
- `trading_plan`（网格、定投、条件计划）不属于指标买卖规则：审阅卡只说明可理解的计划行为，不展示基准价、格距、股数等执行参数；标题不得拼接底层建仓或成交条款。首层仍须使用与普通条件一致的流程卡（连续编号、买/卖/区间各有边框），不能因隐藏参数而退化成无边界段落。参数页不渲染空的“交易规则与参数”买/卖栏，而是在“高级设置”内展示“交易计划与参数”。交易计划内的费率、滑点和最低佣金是该计划唯一可编辑的成本口径；不得再显示一套不参与计划回测的通用“成交与费用”或“研究执行参数”。普通条件策略才使用“交易规则与参数”。
- 320 / 390 / 768 / 1280px 均不得出现横向滚动；长指标名、长条件和中英文混排必须自然换行，不能通过放大字号或控件高度来填满宽屏。
...
| 交易策略超额（个百分点） | 策略总收益 - 基准总收益；不计算或展示净值比值形式的复合相对收益 |

收益比较统一使用三块相扣的拼图：策略收益率 − 买入后一直持有收益率 = 交易策略超额。数字沿用页面数值字体与红涨绿跌语义色，标签沿用正文字体；手机保留三块顺序，单位可换行。结论和曲线使用同一百分点差口径；没有可审计的可比基准时展示不可比较原因，不补零。
