...
- 当前条件和回测区间默认展开；成交、费用、容量、预热、结算延长等低频研究设置归入“高级研究设置”并默认折叠。
- 会改变成交次数的执行语义不得因折叠而消失。高级研究设置必须明确显示“触发口径”和“后续买入”：上穿/下穿/金叉/死叉只在新事件发生时触发，条件持续成立不重复触发；采用 `accumulate_on_new_entry_signal` 时，持仓期间出现新的同类触发可以再次买入。
