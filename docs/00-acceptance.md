...
- [x] `local-real-data-verified`：Event/Composite v2 `composite:1f26afb8b1223b656397abd5c99ce5a0ac9ed6593c3dbf7bdc46fb5a61bb9b8b` 已被当前 strict loader pin 为 `snapshot:4bd9ae887af1cac159bf2d831313c73d839a313b11a746c015ea19fa3d5728cd`；本地 `.env` 已指向该 Composite。
- [x] `local-real-data-verified`：该快照在 2021-02-07 至 2026-08-20 的 9 页/862 条公告查询中固定 26 条 `vendor_observed + validated` 秒级观察（业绩预告 3、业绩快报 2、年报 6、半年报 5、季报 10）；这只证明该股票、区间和五个 event code。
...
- [x] 历史迁移记录：`events:f22b9a…a1171fef` / `composite:1b3dbee…f5a6f053` 及 `0e902577…` 下的 Choice `10002004` 失败只作故障演进证据，不是当前 producer 身份。
...
- [x] `implemented / fixture-tested`：只有明确的 transport 瞬态故障才最多重试 3 次；权限、Schema、业务数据、非交通层异常不重试。POST 使用稳定幂等键，同一逻辑请求重放不得创建第二条 draft/run；尝试次数和请求身份进入审计。
...
- [ ] `Live-unverified`：当前 Composite 的 26 条 observation 中 `document_text=0`。正文策略必须使用显式 opt-in 正文抽取重新采集并发布新的内容寻址 Event v2 + Composite v2；在此之前 loader 对需要正文的策略必须 fail closed。
- [ ] `production-unavailable`：东方财富公告页原始响应字节尚未归档；当前只保存逐页 hash，不能声称不可变原始存储已完成。
- [ ] `Live-unverified`：当前五类的 26 条本地真实观察尚未达到每个事件码的 Demo 门禁：至少 30 个独立真实正例、30 个困难负例（至少 10 个发行人、3 个年份，覆盖更正/撤回/错股/候选/传闻）和 10 条完整因果 E2E。生产声明至少 60+60，并要求独立复核与关键时点/错股零容忍。
...
## 4. 最终 Live 验收：先前公网 API 已有证据，最终 clean-SHA H5 未完成

- [x] 先前公网 API 证据（不属于最终验收 SHA）：revision `4dc55904f38d4d26b6ed3e29fe8a729f529da66c` 上的技术 `run:b2778dc334cc4eb3b39ac9eab2efd54f` 和事件 `run:12fe45b6624249d5a6bba3458751cc98` 再次通过，且引用 `1f26…/4bd9…`。这只证明旧 revision 的公网 API，不替代当前工作树和 H5 浏览器验收。
...
在这些项目全部完成前，文档不得预填 final clean-SHA run ID、收益、回撤或“最终验收通过”。已经核验的旧 revision 公网 API run 可以作为明确标注版本的历史证据；旧 v1 run 仍只作迁移证据，二者都不能冒充最终 H5 验收。
