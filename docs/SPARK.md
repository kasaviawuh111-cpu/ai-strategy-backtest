...
- 2026-09-01 CloudBase 发布运维 P1：后端候选版 `010` 已承接 100% 流量，但平台灰度状态异常导致 `promote` 连续失败，尚未被 CloudBase 标记为 stable。原 `59ac…` WorkBuddy 链接无法覆盖，新 Live `web/dist` 发布于 `8ae96…`；新 origin 必须进入后端精确 CORS 白名单，否则浏览器预检会返回 400。当前决策是不回滚、不继续用 `promote` 阻断发布，之后单独清理灰度/稳定版标记。公网前后端未完成最小连通验证前仍是 `Live-unverified`，不得写成生产验收通过。
