...
def test_success_access_log_contains_response_request_id_without_query_or_body(
    ...
    with (
        caplog.at_level(logging.INFO, logger="ashare_lab.api.access"),
    ...
    record = next(item for item in caplog.records if item.name == "ashare_lab.api.access")
...
def test_rejected_request_is_also_correlated_in_access_log(
    ...
    rendered = "\n".join(
        item.getMessage() for item in caplog.records if item.name == "ashare_lab.api.access"
    )
