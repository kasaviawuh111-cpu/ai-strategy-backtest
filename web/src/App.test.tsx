...
describe('formal main.tsx App journey', () => {
  ...
  it('guides an opinion into explicit A-share strategy choices before allowing a backtest', async () => {
    ...
    const proposals = screen.getByRole('group', { name: '可选规则' })
    const researchEvidence = screen.getByLabelText('联网分析依据')
    expect(researchEvidence).not.toHaveAttribute('open')
    expect(researchEvidence).not.toHaveTextContent('不是回测行情')
    expect(proposals.nextElementSibling).toBe(researchEvidence)
    expect(input).toHaveValue('')
