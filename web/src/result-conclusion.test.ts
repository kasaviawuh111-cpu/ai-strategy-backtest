...
describe('numericResultConclusion', () => {
  it('keeps the conclusion short while the report renders the detailed execution note separately', () => {
    ...
    expect(text).toBe('策略收益 0.00%，本次没有已成交买入，无法计算收益率差。')
  ...
  it.each([
    [{ total: 12, bench: 8, excess: 4 }, '策略盈利 12.00%，同样的钱买入后一直持有盈利 8.00%，收益率领先 4.00 个百分点。'],
    [{ total: 5, bench: 8, excess: -3 }, '策略盈利 5.00%，同样的钱买入后一直持有盈利 8.00%，收益率落后 3.00 个百分点。'],
    [{ total: -10, bench: -3, excess: -7 }, '策略亏损 10.00%，同样的钱买入后一直持有亏损 3.00%，收益率落后 7.00 个百分点。'],
  ...
  it('does not infer a missing benchmark and makes zero completed trades explicit', () => {
    expect(numericResultConclusion(metrics({
      ...
      '策略收益 0.00%，没有可审计的买入持有基准，无法计算收益率差；完整买卖 0 回合。',
    )
  })
  ...
  it('does not call an all-cash strategy outperformance when no buy filled', () => {
    expect(numericResultConclusion(metrics({
      ...
      '策略收益 0.00%，本次没有已成交买入，无法计算收益率差（同样的钱买入后一直持有盈利 8.70%，仅作参考）；完整买卖 0 回合。',
    )
  })
})
