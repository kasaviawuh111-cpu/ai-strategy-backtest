...
当前 Vitest 基线为 13 个测试文件、154 个测试；覆盖正式 `main.tsx → src/App.tsx` 入口、动态能力合同、Live 编译与回测能力解耦、Mock、公网构建门禁、观点→假设→当前 A 股代理→候选选择后重新编译、截图原句的年报正文词频策略、一次澄清信息保留、策略确认、连续历史回合、首页本金隐藏、净值/回撤点位和完整因果轨迹。
